...
namespace NtfsSharp.Drivers.Physical
{
    public class PhysicalDiskDriver : BaseDiskDriver
    {
        ...
        private void ReadMasterBootRecord()
        {
            ...
            var mbrBytes = ReadFile(LogicalBlockAddressSize);
            _masterBootRecord = mbrBytes.ToStructure<MasterBootRecord>();

            if (_masterBootRecord.EndOfSectorMarker != 0xAA55)
        ...
        {
            if (_masterBootRecord.PartitionEntries[0].SystemID != SystemId.GptProtectiveMbr)
                return;
        ...
        private void SelectPartition(uint partition)
        {
            if (GuidPartitionTable == null)
            {
                ...
                var partitionEntry = _masterBootRecord.PartitionEntries[partition];

                _partitionStartSector = partitionEntry.RelativeSector;
                _partitionEndSector = partitionEntry.RelativeSector + partitionEntry.TotalSectors;
            }
            else
            {
                ...
                _partitionStartSector = (uint) efiPartitionEntry.FirstLba;
                _partitionEndSector = efiPartitionEntry.LastLba;
            }
        }
        ...
        {
            public readonly BootIndicator BootIndicator;
